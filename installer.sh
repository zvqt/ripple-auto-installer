...
if [ "$q" = "y" ]; then
	...
	echo "tmux new-session -d -s lets 'tmux set remain-on-exit on && cd lets && python3.5 lets.py'" >> tmux-start.sh
	echo "tmux new-session -d -s bancho 'tmux set remain-on-exit on && cd pep.py && python3.5 pep.py'" >> tmux-start.sh
